#lang eopl
; Name : Ahmet Hakan Hafif
; ID : 64092
; PARTNER NAME AND ID : Kerem Aksoy 64243
;;;;;;;;;;;;;;;;;;; PROBLEMS ;;;;;;;;;;;;;;;;;;;;
;; PROBLEM 1
;; Part A
...
(define (apply-env env search-var)
  (cond
      ...
      ((eqv? (car env) 'extend-env)
        (let ((saved-var (car (cadr env)))
              (saved-val (cadr (cadr env)))
              (saved-env (caddr env)))
...
(define (has-binding? env search-var)
  (cond
    ((eqv? (car env) 'empty-env) #f)
      ((eqv? (car env) 'extend-env)
        (let ((saved-var (car (cadr env)))
              (saved-val (cdr (cadr env)))
              (saved-env (caddr env)))
          (if (eqv? saved-var search-var) #t
...
;!!!!!!!!!!!!!! I implemented this but it outputsarranges in a normal order instead of reverse order. !!!!!!!!!!!!
(define (extend-env* var-list val-list env)
    (if (null? var-list) env 
    (list 'extend-env (list (car var-list) (car val-list)) (extend-env* (cdr var-list) (cdr val-list) env))))
...
(define (apply-rib-env env search-var)
  (cond
      ...
      ((eqv? (car env) 'extend-rib-env)
        (let ((var-list (car (cadr env)))
              (value-list (cadr (cadr env)))
              (saved-env (caddr env)))
          (if (boolean? (checkIn var-list value-list search-var)) (apply-rib-env saved-env search-var) 
              (checkIn var-list value-list search-var))))
      (else
       (display "invalid env"))))

(define (checkIn var-list value-list search-var)
  (cond
      ((null? var-list) #t)
      ((eqv? (car var-list) search-var) (car value-list))
      (else (checkIn (cdr var-list) (cdr value-list) search-var))))
